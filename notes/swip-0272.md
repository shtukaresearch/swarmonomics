...
function manageStake(bytes32 _setNonce, uint256 _addAmount, uint8 _height) external whenNotPaused {
      ...
      /*  
          lookAheadStakes[msg.sender] = Stake({
              overlay: _newOverlay,
 ...
 // New modifier: onlyUpdater
  ...
  function _updateStake internal (
      address owner, 
      bytes32 overlay, 
      uint committedStake, 
      uint potentialStake, 
  ...
  function applyUpdates(address owner) {
      while () {
          if (!req = updateQueue.pop(owner)) break;
          this.call(req);
      }
  }
  
    function lookAheadOverlay(address owner) external view returns bytes32;
    // Return the overlay address that would be in effect after all updates are applied.
    // This needs to be called by the lookAheadInProximity method used by Redistribution:reveal().
    
    function lookAheadCommittedStake(address owner) external view returns uint256;
    // Return the committedStake balance that would be in effect after all updates are applied
    // Required by withdrawFromStake.
    
    function lookAheadPotentialStake(address owner) external view returns uint256;
    // Return the potentialStake balance that would be in effect after all updates are applied
    // Required by migrateStake.
    
    function lookAheadHeight(address owner) external view returns uint8;
    // Return the height that would be in effect after all updates are applied
    // This needs to be called by the lookAheadInProximity method used by Redistribution:reveal().
